...
class DatasetProcess:
    def __init__(self, data_dir: str, train: bool, del_file: bool = True, device: str = 'cuda:2') -> None:
        """
        处理数据集
        ...
        if os.path.exists(self.processed_csv_dir):
            ...
            print(f"'{self.processed_csv_dir}' 不存在, 正在处理csv文件...")
            self.df = None
            self.wt_info = None
            self.mut_info = None
        ...
        for idx, row in tqdm(self.df.iterrows(), total=len(self.df), desc=f"正在处理 {self.dataset_name}"):  # 直接遍历DataFrame
            pdb_id = row['pdb_id']

            # 处理野生型（每个PDB仅一次）
            if pdb_id not in processed_pdbs:
                wt_seq = row['wt_seq']
                wt_feat = self.extract_feature_from_ism(row['wt_seq'], s_index=row['index'])
    ...
    def _process_csv(self):
        ...
        if 'DDG' in df.columns and 'ddg' not in df.columns:
            df = df.rename(columns={'DDG': 'ddg'})
        if 'PDB' in df.columns and 'pdb_id' not in df.columns:
            df = df.rename(columns={'PDB': 'pdb_id'})
        if 'SEQ' in df.columns and 'wt_seq' not in df.columns:
            df = df.rename(columns={'SEQ': 'wt_seq'})
        if 'MUT' in df.columns and 'mut_info' not in df.columns:
            df = df.rename(columns={'MUT': 'mut_info'})
        ...
        if not (n_seq_per_pdb == 1).all():  # 检查n_seq_per_pdb中是否所有的值都等于1
            ...
            print(f'警告: 发现同一PDB对应多个不同的野生蛋白质序, 将其移除 {pdb_with_few_seq}')
        ...
        # 9.打印处理结果
        print(f"保存 {len(self.wt_info)} 个蛋白质 ({len(self.df)} 个突变) 至 '{self.processed_csv_dir}'")
    ...
    def extract_feature_from_ism(self, seqs, ddg=None, s_index=None):
        """
        从ism模型中提取特征
        :param seqs: 蛋白质序列
        :param ddg: 突变ddg
        ...
        """
        # 确保输入格式正确
        assert isinstance(seqs, str), "输入必须是一条蛋白质字符串"
        ...
        return {
            ...
            "ddg": float(ddg) if ddg is not None else None,  # 突变ddg
            "s_index": s_index  # 样本索引
        }
...
def generate_mut_seq(row):
    ...
    if ':' in row['mut_info']:
        split_sign = ':'
    elif ';' in row['mut_info']:
        split_sign = ';'
    else:
        # split_sign = ':'
        raise ValueError("请采用:或;分割突变")
    # 检查mut_info中存在几个突变，根据":"分割
    mut_info_list = row['mut_info'].split(split_sign)
    for mut_info in mut_info_list:
        # 假设mut_info格式为 "A123T"，表示将A替换为T，位置为123（注意：位置从1开始）
        pos = int(mut_info[1:-1]) - 1  # 转换为0-based索引
        # pos = int(mut_info[1:-1])  # 转换为0-based索引
        # 检查位置是否有效
        if pos < 0 or pos >= len(wt_seq):
            raise ValueError(f"第{row['index']+1}个样本的突变位置 {pos+1} 超出序列长度 {len(wt_seq)}")
        # 检查是否存在相同的突变
        if wt_seq[pos] == mut_info[-1]:
            raise ValueError(f"第{row['index']+1}个样本的突变位置 {pos+1} 上的突变 {mut_info} 与WT序列相同")
    ...
    for mut_info in mut_info_list:
        ...
        if wt_seq[pos] != mut_info[0]:
            raise ValueError(f"第{row['index']+1}个样本的突变位置 {pos+1} 上的突变 {mut_info} 与WT序列不同")
