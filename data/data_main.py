...
class DataMain(pl.LightningDataModule):
    ...
    def create_dataset(self, file, apply_thermodynamic_reversibility, fold_dir=None, del_folder=True):
        """
        创建数据集
        :param file: 数据集文件路径
    ...
    def test_dataloader(self):
        ...
        for test_name in self.test_name.split(','):
            test_set = self.create_dataset(test_name, apply_thermodynamic_reversibility=False, del_folder=self.del_files)
...
def pair_pad_collate_fn(batch, contact_cutoff=None):
    ...
    wt_nodes, mut_nodes, masks, ddgs, s_indices = [], [], [], [], []
    # wt_seq, mut_seq = [], []

    max_len = max(max(item[0]['node'].size(0), item[1]['node'].size(0)) for item in batch)

    for wt_info, mut_info, ddg_label, s_index in batch:
    ...
    ddgs = torch.tensor(ddgs, dtype=torch.float32)  # [B]
    s_indices = torch.tensor(s_indices, dtype=torch.int64)  # [B]
    if contact_cutoff is not None:
        # 构建 Batch 对象
    ...
    return wt_data, mut_data, masks, ddgs, s_indices
...
class CustomDataset(Dataset):
        ...
        if path not in self.cache:
            ...
            if ddg_label is None:
                ...
                self.cache[path] = {
                    ...
                    's_index': data['s_index'].item() if isinstance(data['s_index'], torch.Tensor) else data['s_index']
                }
                if contact_map is not None:
                    self.cache[path]['contact'] = contact_map.squeeze(0).detach()
                return self.cache[path]
    ...
    def __getitem__(self, idx):
        ...
        s_index = mut_data.get('s_index', None)
        ...
        # 反对称处理
        if not reverse:
            return wt_dict, mut_dict, mut_ddg, s_index
        else:
            return mut_dict, wt_dict, -mut_ddg, s_index
